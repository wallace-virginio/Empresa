...
public class FuncionarioService {
	...
	public List<Funcionario> showAllFuncionarios() {
		// ao acessar o FuncionarioRepository temos acesso as métodos do JPA como o
		// findAll()
		return FuncionarioRepository.findAll();
	}

	// o segundo serviço que vamos implementar é listar apenas um Funcionario
	// através do ID dele (ra_Funcionario)
	// retorno do tipo Optional - evita o erro de new point exception. Verifica se o
	// Funcionario existe no BD, se não, lança uma exceção
	...
	public Funcionario BuscarUmFuncionario(Integer id_Funcionario) {
		...
		return Funcionario.orElseThrow(
				() -> new ObjectNotFoundException("Objeto não cadastrado! O ID procurado foi.." + id_Funcionario));
	}
